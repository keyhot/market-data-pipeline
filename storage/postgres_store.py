...
# timeout is 30s, so the caller gates this behind a `postgres_readable()`
# that already confirmed the DB reachable (api/main.py) — that gating is
...
def world_liveness_times(
    ...
    """Newest signal, and newest world event that is NOT generated by the
    broadcast apparatus itself (world.salience.BROADCAST_APPARATUS_EVENT_TYPES
    — stream lifecycle, director actions, YouTube broadcast lifecycle). The
    exclusion is the point: during KI-057 the only events still being written
    were the watchdog's own `stream_dropped` rows, and a director or
