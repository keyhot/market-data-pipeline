...
def _shading_prelude(source: str) -> str:
    ...
    `rimStyle`/`rimStroke`/`rimStrokeCircle`/`BODY_RIM_SHADED` (Task 8 put
    `rimStyle` between `paint` and the stroke, so a mood can reach the rim's
    own albedo). Pulled from the real
